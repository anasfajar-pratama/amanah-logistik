...
const menuItems = [
  { to: '/admin/settings', icon: Settings, label: 'Pengaturan', desc: 'Nama perusahaan, WhatsApp, tagline', color: 'bg-accent-50 text-slate-700' },
  { to: '/admin/homepage', icon: Home, label: 'Homepage', desc: 'Hero section, statistik, CTA', color: 'bg-blue-50 text-blue-700' },
  { to: '/admin/services', icon: Layers, label: 'Layanan', desc: 'Kelola layanan pengiriman', color: 'bg-accent-50 text-accent-700' },
...
export default function Dashboard() {
  ...
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-2xl font-black text-primary-800">Dashboard</h1>
        <p className="text-slate-500 mt-1">Selamat datang kembali, <span className="font-semibold text-slate-700">{user?.name}</span>!</p>
      </div>

      {unread > 0 && (
        <div className="bg-accent-50 border border-accent-200 rounded-2xl p-4 mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-accent-50 rounded-xl flex items-center justify-center">
              <MessageSquare className="w-5 h-5 text-accent-600" />
            </div>
            <div>
              <p className="font-bold text-accent-900">Ada {unread} pesan belum dibaca</p>
              <p className="text-accent-700 text-sm">Pesan dari pengunjung website Anda</p>
            </div>
          </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-6 flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="font-bold text-primary-800">Website Anda Aktif</p>
          <p className="text-slate-500 text-sm">Semua perubahan akan langsung tampil di website.</p>
        </div>
        <a href="/" target="_blank" rel="noopener noreferrer"
          className="flex items-center gap-2 text-sm font-semibold text-slate-700 bg-accent-50 hover:bg-accent-100 px-4 py-2 rounded-lg transition-colors">
          Lihat Website <ExternalLink className="w-3.5 h-3.5" />
        </a>
      </div>

      <h2 className="text-lg font-bold text-primary-800 mb-4">Kelola Konten</h2>
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {menuItems.map(({ to, icon: Icon, label, desc, color }) => (
          <Link key={to} to={to}
            className="bg-white border border-gray-100 rounded-2xl p-5 hover:shadow-md hover:border-accent-200 transition-all duration-200 group">
            <div className={`w-12 h-12 rounded-xl flex items-center justify-center mb-4 ${color}`}>
              <Icon className="w-5 h-5" />
            </div>
            <h3 className="font-bold text-primary-800 mb-1 group-hover:text-accent-600 transition-colors">{label}</h3>
