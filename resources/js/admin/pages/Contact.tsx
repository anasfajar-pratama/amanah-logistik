...
export default function ContactPage() {
  ...
  const field = (key: keyof ContactData, label: string, type: 'text' | 'email' | 'tel' | 'textarea' = 'text', hint?: string) => (
    <div key={key}>
      ...
      {type === 'textarea' ? (
        <textarea rows={3} value={data[key]} onChange={e => setData(d => ({ ...d, [key]: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400 resize-none" />
      ) : (
        <input type={type} value={data[key]} onChange={e => setData(d => ({ ...d, [key]: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-orange-400" />
      )}
  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        ...
        <div><h1 className="text-2xl font-black text-slate-900">Kontak</h1><p className="text-slate-500 text-sm">Info kontak dan peta lokasi</p></div>
      </div>
      <form onSubmit={handleSave} className="space-y-6">
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          <h2 className="font-bold text-slate-900 pb-3 border-b border-gray-100">Informasi Kontak</h2>
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          <h2 className="font-bold text-slate-900 pb-3 border-b border-gray-100">Google Maps</h2>
        ...
        <div className="flex items-center gap-4">
          <button type="submit" disabled={saving} className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-60 text-white font-bold px-6 py-3 rounded-xl">
