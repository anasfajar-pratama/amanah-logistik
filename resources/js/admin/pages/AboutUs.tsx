  ...
  return (
    <div>
      <div className="flex items-center gap-3 mb-8">
        ...
        <div><h1 className="text-2xl font-black text-primary-800">Tentang Kami</h1><p className="text-slate-500 text-sm">Profil dan visi perusahaan</p></div>
      </div>
      <form onSubmit={handleSave} className="space-y-6">
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          <h2 className="font-bold text-primary-800 pb-3 border-b border-gray-100">Konten</h2>
          <div><label className="block text-sm font-semibold text-slate-700 mb-1.5">Judul</label>
            <input value={data.title ?? ''} onChange={e => setData(d => ({ ...d, title: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400 focus:border-transparent" /></div>
          <div><label className="block text-sm font-semibold text-slate-700 mb-1.5">Paragraf 1</label>
            <textarea rows={3} value={data.description_1 ?? ''} onChange={e => setData(d => ({ ...d, description_1: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400 focus:border-transparent resize-none" /></div>
          <div><label className="block text-sm font-semibold text-slate-700 mb-1.5">Paragraf 2</label>
            <textarea rows={3} value={data.description_2 ?? ''} onChange={e => setData(d => ({ ...d, description_2: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400 focus:border-transparent resize-none" /></div>
          <div><label className="block text-sm font-semibold text-slate-700 mb-1.5">Visi</label>
            <textarea rows={2} value={data.vision ?? ''} onChange={e => setData(d => ({ ...d, vision: e.target.value }))} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400 focus:border-transparent resize-none" /></div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          <h2 className="font-bold text-primary-800 pb-3 border-b border-gray-100">Poin Keunggulan (Bullet)</h2>
          <ul className="space-y-2">
            {(data.highlights ?? []).map((h, i) => (
              <li key={i} className="flex items-center gap-3 bg-accent-50 rounded-xl px-4 py-2.5">
          ...
          <div className="flex gap-2">
            <input value={newHighlight} onChange={e => setNewHighlight(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addHighlight(); } }}
              placeholder="Tambah poin baru..." className="flex-1 border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-accent-400" />
            <button type="button" onClick={addHighlight} className="bg-accent-50 hover:bg-accent-100 text-slate-700 px-4 py-2.5 rounded-xl font-medium text-sm flex items-center gap-1"><Plus className="w-4 h-4" />Tambah</button>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-4">
          <h2 className="font-bold text-primary-800 pb-3 border-b border-gray-100">Foto</h2>
          {imagePreview && (
          ...
          <button type="button" onClick={() => fileRef.current?.click()} className="flex items-center gap-2 border-2 border-dashed border-gray-200 rounded-xl px-6 py-4 text-slate-500 hover:border-accent-400 hover:text-accent-500 transition-colors w-full justify-center text-sm">
        ...
        <div className="flex items-center gap-4">
          <button type="submit" disabled={saving} className="flex items-center gap-2 bg-accent-500 hover:bg-accent-600 disabled:opacity-60 text-white font-bold px-6 py-3 rounded-xl transition-colors">
